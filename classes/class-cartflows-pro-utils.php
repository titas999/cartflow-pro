...
class Cartflows_Pro_Utils {
	...
	public function get_checkout_id_from_data( $post_data ) {
		...
		if ( isset( $post_data['_wcf_checkout_id'] ) ) {
			...
			$checkout_id = filter_var( wp_unslash( $post_data['_wcf_checkout_id'] ), FILTER_SANITIZE_NUMBER_INT );
	...
	public function get_flow_id_from_data( $post_data ) {
		...
		if ( isset( $post_data['_wcf_flow_id'] ) ) {

			$flow_id = filter_var( wp_unslash( $post_data['_wcf_flow_id'] ), FILTER_SANITIZE_NUMBER_INT );
	...
	public function get_fragments( $new_key, $data = array() ) {
		...
		if ( isset( $data['checkout_id'] ) ) {
			$flow_id = wcf()->utils->get_flow_id_from_step_id( $data['checkout_id'] );

			// Check if instant layout is enabled.
			if ( Cartflows_Pro_Helper::is_instant_layout_enabled( intval( $flow_id ) ) ) {
				// Set force_update_fragment to true.
				$data['force_update_fragment'] = true;
			}
		}
		...
		$response = array(
			...
			'force_update'  => isset( $data['force_update_fragment'] ) ? $data['force_update_fragment'] : false,
	...
	public function get_offer_data( $step_id, $selected_product_id = '', $input_qty = '', $order_id = 0 ) {
		...
		$product_id = $this->get_variable_variation_product_id( $product_id );
	 ...
	 *
	 * @param string           $discount_type discount type.
	 * @param float|int|string $discount_value discount value.
	 * @param float|int|string $product_price product price.
	...
	public function get_calculated_discount( $discount_type, $discount_value, $product_price ) {
		...
		// Ensure product price is valid and non-zero.
		if ( empty( $product_price ) || ! is_numeric( $product_price ) ) {
			return '';
		}

		// Ensure discount type and value are valid and non-zero.
		if ( empty( $discount_type ) || empty( $discount_value ) || ! is_numeric( $discount_value ) ) {
			return '';
		}
	...
	public function may_be_append_query_string( $original_query_strings ) {
		 ...
		 *
		 * Use-case: If the `may_be_append_query_string` is available i:e in the free version then call this function.
		 *
		 * Remove this whole function in the next major update. Possibility in 2.0
		 */
		if ( method_exists( wcf()->utils, 'may_be_append_query_string' ) ) {
			return wcf()->utils->may_be_append_query_string( $original_query_strings );
		}
	...
	/**
	 * Generates a template for displaying license restriction messages.
	 *
	 * This function generates a template for displaying messages related to license restrictions.
	 *
	 * @param array $texts An array containing the template data.
	 * @return string|false The generated template HTML.
	 */
	public function get_license_restriction_template( $texts ) {
		ob_start();
		?>
		<div class="wcf-license-restriction-box">
			<div class="wcf-license-restriction-box-title"><?php echo esc_html( $texts['title'] ); ?></div>
			<?php foreach ( $texts['messages'] as $message ) { ?>
				<div class="wcf-license-restriction-box-message"><?php echo esc_html( $message ); ?></div>
			<?php } ?>
			<a class="wcf-license-restriction-button" href="<?php echo esc_url( admin_url( 'admin.php?page=cartflows&settings=1&tab=license' ) ); ?>" target="_blank">
				<?php echo esc_html__( 'Activate License', 'cartflows-pro' ); ?>
			</a>
		</div>
		<?php
	...
	public function get_formatted_product_price_range( $product ) {
		// Return early if not a valid WC_Product object.
		...
		if ( 'variable' === $product_type ) {
			// Check method existence for compatibility with product type.
			if ( method_exists( $product, 'get_variation_prices' ) ) {
				...
				if ( isset( $variation_price_range['price'] ) && is_array( $variation_price_range['price'] ) && ! empty( $variation_price_range['price'] ) ) {
					$min_price = min( $variation_price_range['price'] );
					$max_price = max( $variation_price_range['price'] );
				}
				if ( isset( $variation_price_range['regular_price'] ) && is_array( $variation_price_range['regular_price'] ) && ! empty( $variation_price_range['regular_price'] ) ) {
					$min_reg_price = min( $variation_price_range['regular_price'] );
					$max_reg_price = max( $variation_price_range['regular_price'] );
				}

				// Fallback if price values are still null or nonnumeric.
				if (
					isset( $min_price, $max_price )
					&& is_numeric( $min_price ) && is_numeric( $max_price )
					&& $min_price !== $max_price
				) {
					$original_price = html_entity_decode( wp_kses_post( wc_format_price_range( (string) $min_price, (string) $max_price ) ) );
				}

				// If all regular prices are same as sale price range, prefer to show that.
				if (
					isset( $min_reg_price, $max_reg_price )
					&& is_numeric( $min_reg_price ) && is_numeric( $max_reg_price )
					&& $min_reg_price !== $max_reg_price
					&& ! empty( $original_price )
				) {
					$original_price = html_entity_decode( wp_strip_all_tags( wc_format_price_range( (string) $min_reg_price, (string) $max_reg_price ) ) );
				}

				// If price range is not set, try to get single minimum price value for variable product.
				if ( empty( $original_price ) ) {
					$price          = $product->get_price();
					$original_price = ! empty( $price ) ? $price : $product->get_regular_price();
					$original_price = html_entity_decode( wp_strip_all_tags( wc_price( (float) $original_price ) ) );
				}
			} else {
				// Defensive fallback: get_price should exist, fallback to that for unknown/other variable product classes.
				$price          = $product->get_price();
				$original_price = ! empty( $price ) ? $price : $product->get_regular_price();
				$original_price = html_entity_decode( wp_strip_all_tags( wc_price( (float) $original_price ) ) );
			}
