...
class Cartflows_Pro_Analytics_Tracking {
	...
	/**
	 * CartFlows analytics NameSpace
	 *
	 * @var string
	 */
	private $namespace = 'cartflows-pro/v1';

	/**
	 * CartFlows REST API base,
	 *
	 * @var string
	 */
	private $rest_base = '/flow-analytics/';
	...
	public function save_analytics_data( $request ) {
		...
		if ( Cartflows_Compatibility::get_instance()->is_page_builder_preview() ) {
			$response = array(
				'message' => __( 'Page is opened in a preview mode.', 'cartflows-pro' ),
			);
		...
		if ( ! $this->is_valid_thank_you_page( $current_step, $url_params ) ) {
			$response = array(
				'message' => __( 'Page is opened directly without placing an order.', 'cartflows-pro' ),
			);
		...
		$prev_control_id = $wcf_step_obj->get_prev_control_id_for_analytics( $current_flow, $flow_cookie_data );
	...
	public function add_localize_vars( $localize ) {
		...
		$localize['analytics_base_url'] = esc_url( get_home_url() ) . '/wp-json/' . $this->namespace . $this->rest_base; // The URL will be: '/cartflows-pro/v1/flow-analytics/'.
