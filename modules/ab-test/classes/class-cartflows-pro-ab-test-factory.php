...
class Cartflows_Pro_Ab_Test_Factory {
	 ...
	 *
	 * Filters out disabled variations and redistributes traffic among enabled ones.
	 *
	...
	public function override_variation_content_to_control( $variation_id ) {
		...
		if ( $variant && 'publish' === $variant->post_status ) {
			...
			// Update the global wcf_step to reflect the actual variation being shown.
			// This ensures views and conversions are tracked against the correct variation
			// when "Override Permalink for A/B Test" setting is enabled.
			$GLOBALS['wcf_step'] = wcf_pro_get_step( $variation_id ); //phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		}
	}
}
