...
class FormFields extends AjaxBase {
	...
	public function prepare_custom_field() {
		...
		$file_size = isset( $_POST['file_size'] ) ? absint( $_POST['file_size'] ) : 5;
		// Accepted File types will be sanitized in the helper function as it accepts comma separated values.
		$accepted_file_types = isset( $_POST['accepted_file_types'] ) ? \Cartflows_Helper::sanitize_accepted_file_types( wp_unslash( $_POST['accepted_file_types'] ) ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
